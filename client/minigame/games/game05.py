...
def draw_text_BLACK_score(surf, text, size, x, y): #把文字寫到畫面上的動作；第一格是要寫在什麼平面上，第二格是要寫的文字，第三格是文字大小，第四、五格是要寫在哪裡(座標)
    font = pygame.font.Font(font_name, size) #(字體,文字大小)
...
class Rock(pygame.sprite.Sprite):
    ...
    def rotate(self):
        ...
        self.image = pygame.transform.rotate(self.image_ori, self.total_degree) #第一格是要轉動的圖片(讓沒有失真過的圖片轉動)；第二格是要轉動幾度
...
while running: #判斷遊戲是否要繼續
    ...
    hits = pygame.sprite.spritecollide(player, rocks, True, pygame.sprite.collide_circle) #飛船跟石頭撞到時,要不要把石頭刪掉(原本是不用刪，因為遊戲結束，但後來增加了飛船血條，也就是說被砸到不一定遊戲結束，所以石頭要刪);第四格為用圓形框出飛船和石頭，使碰撞判斷更精準
